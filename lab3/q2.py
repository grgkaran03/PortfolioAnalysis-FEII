import time, math, numpy as np,math,matplotlib.pyplot as plt
...
def lookback_option_basic(S0, T, M, r, sigma, display):
  if display == 1: 
    print("\n\n*********  Executing for M = {}  *********\n".format(M))
  curr_time = time.time()
  ...
  if display == 1: 
    print("Initial Price of lookback Option \t= {}".format(option_price[0][0]))
...
prices = []
for m in M:
    prices.append(lookback_option_basic(S0 = 100, T = 1, M = m, r = 0.08, sigma = 0.30, display = 1))

plot_fixed(M, prices, "M", "Initial Option Prices", "Initial Option Prices vs M")
M = [i for i in range(1, 21)]
prices.clear()
for m in M:
    prices.append(lookback_option_basic(S0 = 100, T = 1, M = m, r = 0.08, sigma = 0.30, display = 0))
...
print("\n\n-----------------------  sub-part(c)  -----------------------")
lookback_option_basic(S0 = 100, T = 1, M = 5, r = 0.08, sigma = 0.30, display = 2)
