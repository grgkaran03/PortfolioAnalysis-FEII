...
def compute_cml_equation(mu_rf, mu_market, risk_market):
    slope = (mu_market - mu_rf) / risk_market
    intercept = mu_rf
    return slope, intercept

def plot_efficient_frontier(risk_plot1, returns_plot1, risk_plot2, returns_plot2, risk_market, mu_market, risk_min_var, mu_min_var):
...
def plot_cml(risk, returns, risk_cml, returns_cml):
    plot_fixed_both(risk, returns, risk_cml, returns_cml, "Risk (sigma)", "Returns",
                    "Capital Market Line with Markowitz Efficient Frontier")
...
def main():
    ...
    market_portfolio_weights = (M - mu_rf * u) @ np.linalg.inv(C) / (
            (M - mu_rf * u) @ np.linalg.inv(C) @ np.transpose(u))
    ...
    plot_efficient_frontier(risk_plot1, returns_plot1, risk_plot2, returns_plot2, risk_market, mu_market, risk_min_var,
                            mu_min_var)
    ...
    slope, intercept = compute_cml_equation(mu_rf, mu_market, risk_market)
    print("\nEquation of CML is:")
    print(f"y = {slope:.2f} x + {intercept:.2f}\n")

    plot_cml(risk, returns, risk_cml, returns_cml)
